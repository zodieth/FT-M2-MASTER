...
nombre.innerHTML = nombre.innerHTML + ' Mateo';
...
function buildToDos(toDos) {
  // Tu código acá:
  let array = toDos.map(function (el, index) {
    return buildToDo(el, index);
  })
  return array;
}
...
function addToDo() {
  ...
  if (value !== '') {
    ...
    //1
    let miTodo = new ToDo(value);
    //2
    toDoItems.push(miTodo);
    //3
